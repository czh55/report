                ...
                <div class="well">
                    ...
                    <div id="myTabContent" class="tab-content">
                        <div class="tab-pane active in" id="home">
                            <form id="tab" method="post" action="/report/index.php/Home/Task/save_task">
                            	<!--++++++++++++++++++task+++++++++++++++++++++++++++++-->
                                <label>任务类型</label>
                                <select name="type_id" id="DropDownTimezone" class="input-xlarge">
                                		<?php if(is_array($types)): foreach($types as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; ?>                                     
                         		 </select> 
                                <!-- percent为默认值0，只能通过触发器来改 -->                           
                                <label>任务内容</label>
                                <input type="text" value="" class="input-xlarge" name="content">
                               <!-- date_time1 是默认提交时间-->
                                <label>任务计划截止时间</label>
                                <input type="text" value="" class="laydate-icon input-xlarge" id="start"  name="time_plan">
                                <label>任务说明</label>
                                ...
                                <!--+++++++++++++++++++++schedule++++++++改到触发器中了++++++++++++++++-->
                                <!--task_id 这个得自己从数据库里取出来
                                <label>进度内容</label>
                                <input type="text" value="" class="input-xlarge" name="content_schedule">
                               	commit_time 这个是数据库默认的-->
